...
class MTUCoursesConverter {
    ...
    fun StringToMTUCourses(string: String?): MutableList<MTUCourses>? {
        return Gson().fromJson(string, object : TypeToken<MutableList<MTUCourses?>?>() {}.type)
    }
...
@Database(entities = [MTUCoursesEntry::class], version = 1)
@TypeConverters(MTUCoursesConverter::class)
abstract class AppDatabase : RoomDatabase() {
    abstract fun courseDao(): CourseDao
}

@Entity(tableName = "mtucourses", primaryKeys = ["semester", "year"])
